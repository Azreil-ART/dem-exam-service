        ...
        <DataTable v-model:editingRows="editingRows" :value="zayavki" editMode="row" dataKey="id"
            ...
            <Column field="executorFullName" header="Исполнитель" style="width: 15%">
                <template #editor="{ data, field }">
                    <Dropdown v-model="data.executorId" :options="executorList" optionLabel="fullName" optionValue="id"
            ...
            <Column field="comments" header="Комментарии" style="width: 10%">
                <template #body="slotProps">
                    <comment :id="slotProps.data.id"></comment>
                </template>
            </Column>
...
onMounted(async () => {
    ...
    const zayavki_raw = await zavkaStore.getAllZayavki()
